...
def download_model_from_s3(model_filename, bucket_name, folder="model/bestModel"):
    ...
    new_model_filename = "ml_model_regression_s3.pkl"

    s3_client = boto3.client("s3")
    model_key = f"{folder}/{model_filename}"
    local_file_path = f"model/{new_model_filename}"
...
def evaluate_and_compare_models(
    regressor_new, X_test, y_test, model_filename, bucket_name
):
    regressor_best = None
    ...
    if check_model_exists_in_s3(model_filename, bucket_name, folder=source_folder):
        regressor_best = download_model_from_s3(
            model_filename, bucket_name, folder=source_folder
        )

    # Realizar predicciones
    y_pred_best = (
        regressor_best.predict(X_test) if regressor_best else np.zeros(len(y_test))
    )
...
def move_model_to_backup(model_filename, bucket_name, source_folder, backup_folder):
    ...
    try:
        ...
        s3_client.copy_object(
            CopySource=copy_source, Bucket=bucket_name, Key=backup_key
        )
